...
library(DHARMa) #for checking NB GLMM
...
Richness <- fish[fish$DietH != "Other" & fish$DietH != "na",]

all_combos <- expand_grid(
  SurveyNo = unique(Richness$SurveyNo),
  DietH = unique(Richness$DietH))

# Go from abundance to species richness, and bringing back 0 values for groups without observations
## First store metadata
survey_meta <- Richness %>%
  select(SurveyNo, Transect, Depth.MLLW, Reef.dist_m, AR.dist_m, YearGroup, Date, Patch.type, Treatment, Observer) %>%
  distinct()
...
Richness <- Richness %>%
  ...
  mutate(SpeciesRichness = replace_na(SpeciesRichness, 0))
...
# Go from character to numeric for distances, taking NA's into account
Richness$AR.dist_m    <- as.numeric(trimws(Richness$AR.dist_m))
Richness$Reef.dist_m  <- as.numeric(trimws(Richness$Reef.dist_m))

# Make combined variable for taking intra-survey Observer differences into account in the model
Richness$TransectDate <- paste(Richness$Transect, Richness$Date, sep="_")
...
```{r richness}
# Model from paper
modelOG<- lmer(SpeciesRichness ~ Depth.MLLW + Reef.dist_m +  AR.dist_m + YearGroup * Patch.type + DietH + (1 | Transect) +
(1 | TransectDate:Observer), data = Richness) 
# modelpois<- glmer(SpeciesRichness ~ Depth.MLLW + Reef.dist_m +  AR.dist_m + YearGroup * Patch.type + DietH + (1 | Transect) +
# (1 | TransectDate:Observer), data = Richness, family = "poisson") 
model <- glmmTMB(SpeciesRichness ~ Depth.MLLW + Reef.dist_m +  AR.dist_m + YearGroup * Patch.type + DietH + (1 | Transect) +
(1 | TransectDate:Observer), data=Richness, family=nbinom2)

# Look at model
summary(model)

### Post hoc
emmeans(model, list(pairwise ~ YearGroup), adjust='tukey', type = "response") 
...
# check for residual patterns by predictor
# plotResiduals(sim_res, form = ~ DietH)

## Normality
res <- resid(model)
qqnorm(res) 
qqline(res) # Dots are around line, visual inspection says normal distribution
shapiro.test(res) #p>0.05 thus normal distributed
...
#lot(ggpredict(model1, "Treatment"))
#plot(ggpredict(model1, "YearGroup"))
...
ggplot(Biomass_combined, aes(x = Patch.type, y = Mean_Biomass_perSurveyLog10)) +
   ...
   geom_errorbar(data = Biomass_combined,
                 ...
                 width = 0.1, color = "blue") +  # error bars
   labs(title = "Herbivore species Biomass log-transformed",
        x = "Patch type",
        y = "Herbivore species Biomass") +
   theme_minimal()
