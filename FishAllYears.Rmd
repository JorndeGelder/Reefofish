...
## Statistical models
```{r richness}
# Three versions of modelling the data: Gaussian, negative binomial and Conway-Maxwell Poisson. All use the same variables
# Version 1: Gaussian. Leads to non-normal distribution of residuals, and doesn't fit count data well.
modelG<- lmer(SpeciesRichness ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Richness)

# Version 2: Negative binomial with distances including NA values > distance not significant for AR patches > exclude
modelNB <- glmmTMB(SpeciesRichness ~ Depth.MLLW + Reef.dist_m +  AR.dist_m + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=nbinom2)

# Version 3: Negative binomial. Considering underdispersion (UD) gives 0,78 it is a feasible option.
modelNB2 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=nbinom2)

# Version 4: Conway-Maxwell Poisson, but without distances modelled.
modelCMP1 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois())

# Version 5: Conway-Maxwell Poisson, but with dispersion formula
modelCMP2 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois(), dispformula = ~ Patch.type)

# Version 6: v5, but with both distances and no dispersion formula
modelCMP3 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + Reefdistance:Treatment +  ARdistance:Treatment + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois())

# Look at model
Anova(modelNB)

##### Interpretation of results:
## There are three non-significant effects: the covariates Reef distance and AR distance, as well as Patch.type
# Normality, heteroscedasticity with DHARMa
sim_resNB2 <- simulateResiduals(fittedModel = modelNB2, re.form = NULL)
sim_resCMP1 <- simulateResiduals(fittedModel = modelCMP1, re.form = NULL)
sim_resCMP2 <- simulateResiduals(fittedModel = modelCMP2, re.form = NULL)
sim_resCMP3 <- simulateResiduals(fittedModel = modelCMP3, re.form = NULL)
...
# testZeroInflation(sim_res) # <- Not significant
# testCategorical(sim_resCMP1, catPred = Richness$Depth.MLLW)
# testCategorical(sim_resCMP1, catPred = Richness$YearGroup)
# testCategorical(sim_resCMP1, catPred = Richness$Patch.type)
# testCategorical(sim_resCMP1, catPred = Richness$DietH)
```
...
plot(Richness_combined$Depth.MLLW, Richness_combined$SpeciesRichness, xlab = "Depth", ylab = "Species richness", main = "Species richness vs Depth (covariate)")
...
emm_df <- as.data.frame(emmeans(modelCMP3, ~ YearGroup * DietH, type = "response"))
emm_totals <- emm_df %>%
  group_by(YearGroup) %>%
  summarise(
    total = sum(response),
...
testCategorical(Bsim_resLoglink3, catPred = Richness$Depth.MLLW)
testCategorical(Bsim_resLoglink3, catPred = Richness$YearGroup)
testCategorical(Bsim_resLoglink3, catPred = Richness$Patch.type)
testCategorical(Bsim_resLoglink3, catPred = Richness$DietH)
testResiduals(Bsim_resLoglink3)
```
## Plots
```{r}
#emm_df <- as.data.frame(emmeans(BmodelLoglink3, ~ YearGroup * DietH, type = "response"))
...
#testResiduals(sim_resLoglink2)
#testResiduals(sim_resLoglink3)
```
...
testCategorical(Asim_resNB4, catPred = Abundance$Depth.MLLW)
testCategorical(Asim_resNB4, catPred = Abundance$YearGroup)
testCategorical(Asim_resNB4, catPred = Abundance$Patch.type)
testCategorical(Asim_resNB4, catPred = Abundance$DietH)
```
## Plots
```{r}
...
# Make NMDS plot
nmds = metaMDS(BiomassNMDS_data3, distance = "bray")
...
outliers <- coords[order(coords$MDS1, decreasing = TRUE)[1:2], ]
outliers
...
ggplot(data.scores,aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_text(
  data = subset(data.scores, SampleID %in% c("304", "305")),
  aes(label = SampleID),
...
# Start from full data and null out the 2 outliers only for ellipse grouping
data_ell <- data.scores
data_ell$Patch.type[data_ell$SampleID %in% c("304","305")] <- NA  # mask them, don't drop

# Remove any Patch types that now have <2 samples
valid_groups <- names(which(table(data_ell$Patch.type, useNA = "no") >= 2))
data_ell$Patch.type[!(data_ell$Patch.type %in% valid_groups)] <- NA  # mask small groups too
...
# 3. Generate ellipse dataframe from filtered ellipse object
ellipse_df <- do.call(rbind, lapply(names(ell), function(g) {
  df <- as.data.frame(
    veganCovEllipse(ell[[g]]$cov, ell[[g]]$center, sqrt(qchisq(0.99, df = 2)))
  )
...
ggplot(subset(data.scores, !(SampleID %in% c("304", "305"))), 
        aes(x = NMDS1, y = NMDS2)) +
        geom_point(size = 3, aes(colour = Patch.type)) +
 ...
 coord_cartesian(xlim = c(-0.2, 0.1), ylim = c(-0.1, 0.1)) +
  theme(axis.text = element_text(size = 12, colour = "black"),
    axis.title = element_text(size = 14),
    legend.text = element_text(size = 12),
    legend.title = element_text(size = 14),
