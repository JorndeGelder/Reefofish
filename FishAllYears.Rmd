...
fish <- fish %>%
  select(-GPS, -Reef.dist_m, -AR.dist_m, -Year.created, -Area_m2, -Site, -Diet)

# Select relevant surveys
fish <- fish[fish$Patch.type %in% c("CAGE", "CAKE","BRU","COMP","Control","Reference") & fish$Year >= 2021,]

# Set data types
fishtofactors <- c('SurveyNo', 'Treatment', 'Patch.type', 'Transect', 'Observer', 'Year', 'Species', 'DietH') 
...
#fish_unique$YearGroup <- ifelse(fish_unique$Year %in% c(2021, 2022), "2021–2022",
#                   ifelse(fish_unique$Year %in% c(2023, 2024), "2023–2024",
#                   ifelse(fish_unique$Year == 2025, "2025", NA)))
...
# Combine all species each of the six patch types & note down survey count per patch type for average
#SurveyCount <- aggregate (fishRich, by Patch.type, data = fishRich, FUN=length)
#SurveyCount <- select(SurveyCount, c())
#fishRich <- aggregate(fishRich, by Patch.type,data = fishRich, FUN=sum) 

## Average surveys done simultaneously
fishRichsumsubmeans <- fishRichsum 
fishRichsumsubmeans <- aggregate(S ~ Transect + Date + Year, fishRichsumsubmeans, mean)

## Average over replicate transects within each Year 
fishRichsumsubtransmeans <- fishRichsumsubmeans
fishRichsumsubtransmeans <- aggregate(S ~ Transect + Year, fishRichsumsubtransmeans, mean)
