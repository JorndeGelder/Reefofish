...
emm1 <- emmeans(MSmodelLoglink4, ~ YearGroup, type = "response")
cld1 <- cld(emm1, Letters = letters, adjust = "sidak")
emm_df1 <- as.data.frame(cld1)
emm_df1$.group <- trimws(emm_df1$.group)

plot1 <- ggplot(emm_df1, aes(x = YearGroup, y = response)) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.2) +
  geom_point(size = 3) +
  geom_text(aes(label = .group, y = asymp.UCL), vjust = -0.5, size = 4) +
  labs(x = "Year Group", y = "Mean Size (cm)", 
       title = "Mean Size by Year Group") +
  theme_bw()

# Plot 2: DietH main effect (no interaction)
emm2 <- emmeans(MSmodelLoglink4, ~ DietH, type = "response")
cld2 <- cld(emm2, Letters = letters, adjust = "sidak")
emm_df2 <- as.data.frame(cld2)
emm_df2$.group <- trimws(emm_df2$.group)

plot2 <- ggplot(emm_df2, aes(x = DietH, y = response)) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.2) +
  geom_point(size = 3) +
  geom_text(aes(label = .group, y = asymp.UCL), vjust = -0.5, size = 4) +
  labs(x = "Diet", y = "Mean Size (cm)", 
       title = "Mean Size by Diet") +
  theme_bw()

# Plot 3: Patch.type main effect only (interaction not significant)
emm3 <- emmeans(MSmodelLoglink4, ~ Patch.type, type = "response")

cld3 <- cld(emm3, Letters = letters, alpha = 0.10, adjust = "sidak")
emm_df3 <- as.data.frame(cld3)
emm_df3$.group <- trimws(emm_df3$.group)

plot3 <- ggplot(emm_df3, aes(x = Patch.type, y = response)) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.2) +
  geom_point(size = 3) +
  geom_text(aes(label = .group, y = asymp.UCL), vjust = -0.5, size = 4) +
  labs(x = "Patch Type", y = "Mean Size (cm)", 
       title = "Mean Size by Patch Type") +
  theme_bw()

# Plot 5: Three-way interaction (YearGroup × Patch.type × DietH)
emm5 <- emmeans(MSmodelLoglink5, ~ DietH * Patch.type * YearGroup, type = "response")
emm_df5 <- as.data.frame(emm5)
...
emm_df5 <- merge(emm_df5, actual_combinations, by = c("DietH", "Patch.type", "YearGroup"))

# Calculate cld for each combination of Patch.type and DietH that actually exists
cld_list5 <- list()
for (patch in unique(emm_df5$Patch.type)) {
  for (diet in unique(emm_df5$DietH[emm_df5$Patch.type == patch])) {
    # Check if this combination has data
    if (nrow(emm_df5[emm_df5$Patch.type == patch & emm_df5$DietH == diet, ]) > 0) {
...
emm_df5 <- do.call(rbind, cld_list5)

plot5 <- ggplot(emm_df5, aes(x = DietH, y = response, 
  ...
  labs(x = "Diet", y = "Mean Size (cm)", 
       title = "Mean Size by Diet, Patch Type, and Year Group (Three-way Interaction)",
       color = "Year Group") +
  theme_bw() +
...
## Plots
```{r}
# Plot 1: YearGroup with stacked bars for DietH
emm_df1 <- as.data.frame(emmeans(AmodelNB6, ~ YearGroup + DietH, type = "response"))

# Get marginal means for YearGroup for the cld letters
emm1_year <- emmeans(AmodelNB6, ~ YearGroup, type = "response")
cld1_year <- cld(emm1_year, Letters = letters, adjust = "sidak") %>%
  as.data.frame()
...
# Add cld letters
emm1_totals <- emm1_totals %>%
  left_join(cld1_year[, c("YearGroup", ".group")], by = "YearGroup")
...
plot1 <- ggplot(emm_df1, aes(x = YearGroup, y = response, fill = DietH)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_errorbar(
    data = emm1_totals,
...
# Plot 2: YearGroup on x-axis, faceted by Patch.type
emm_df2 <- as.data.frame(emmeans(AmodelNB6, ~ YearGroup * Patch.type + DietH, type = "response"))

# Get marginal means for cld
emm2_year <- emmeans(AmodelNB6, ~ YearGroup | Patch.type, type = "response")
cld2_year <- cld(emm2_year, Letters = letters, adjust = "sidak") %>%
  as.data.frame()
...
# Add cld letters
emm2_totals <- emm2_totals %>%
  left_join(cld2_year[, c("YearGroup", "Patch.type", ".group")], by = c("YearGroup", "Patch.type"))
...
plot2 <- ggplot(emm_df2, aes(x = YearGroup, y = response, fill = DietH)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_errorbar(
    data = emm2_totals,
    aes(x = YearGroup, y = total, ymin = LCL, ymax = UCL),
...
ggplot(site.scores,aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_text(
    data = subset(site.scores, SampleID %in% c("192","175")),
    aes(label = SampleID),
...
data_ell$Patch.type[data_ell$SampleID %in% c("192","175")] <- NA  # mask them, don't drop
...
final_plot <- ggdraw() +
  draw_plot(main_plot) +
  draw_plot(inset_plot, x = 0.55, y = 0.65, width = 0.20, height = 0.30)
...
vectors_segments$pval <- guild_envfit_subset$vectors$pvals
vectors_segments_sig <- subset(vectors_segments, pval < 0.05)
```
