...
# Pomacentrus aquilus is a special case, because it has to get merged with an already existing entry (Chrysiptera sp), being probably misidentified earlier. First we determine whether to add up the Pom aqu counts and Chr sp counts, based on if and when they were observed.
fish.2017_2022 <- fish.2017_2022 %>%
  group_by(SurveyNo) %>%
  mutate(
    sum11 = sum(Abundance[Species == "Chrysiptera sp" & Coloured == 11], na.rm = TRUE),
    sum12 = sum(Abundance[Species == "Chrysiptera sp" & Coloured == 12], na.rm = TRUE)
  ) %>%
  ungroup() %>%
  
  filter(
    Species != "Chrysiptera sp" | (
      # Case 1: both > 0 → remove 11
      !(sum11 > 0 & sum12 > 0 & Coloured == 11) &
      # Case 2: sum11 > 0 & sum12 == 0 → remove 12
      !(sum11 > 0 & sum12 == 0 & Coloured == 12) &
      # Case 3: sum11 == 0 & sum12 > 0 → remove 11
      !(sum11 == 0 & sum12 > 0 & Coloured == 11) &
      # Case 4: both 0 → remove 11
      !(sum11 == 0 & sum12 == 0 & Coloured == 11)
    )
  )%>% select(-sum11, -sum12)  

# Then, in case both were observed during the survey: add abundances up and remove the duplicate row.
fish.2017_2022 <- fish.2017_2022 %>%
  group_by(SurveyNo, Species, SizeClass) %>%
  mutate(
    Abundance = ifelse(Species == "Chrysiptera sp",
                       sum(Abundance, na.rm = TRUE),
                       Abundance)
  ) %>%
  filter(!(Species == "Chrysiptera sp" & duplicated(SizeClass))) %>%
  ungroup()
...
# Select all rows where SizeClass is 65
...
#Remove duplicates in sizeclass 11 column
...
## Cleanup
fish.2023 <- select(fish.2023, - c("a", "b", "Area"))
...
## Cleanup
fish.2024 <- select(fish.2024, - c("a", "b", "Area"))
...
### Update taxonomy for re-named or previously misidentified species
fish.2025$Species <-   ifelse(fish.2025$Species == "Pycnochromis dimidiatus", "Pycnochromis fieldi",fish.2025$Species)
...
## Cleanup
fish.2025 <- select(fish.2025, - c("a", "b", "Area"))
fish.2025 <- fish.2025 %>% drop_na(Abundance_m2) # Drop empty rows for surveys not yet done

# ---- MERGE DATASETS ----
## Make column order match
### 2017-2022
fish.2017_2022 <- dplyr::select(fish.2017_2022, c("SurveyNo", "Species", "SizeClass_11", "SizeClass", "Abundance", "Abundance_m2", "Biomass_kgha",  "Coloured", "Diet", "DietH", "Date", "Location", "Transect", "Observer", "Comments")) 

### 2023
fish.2023 <- dplyr::select(fish.2023, c("SurveyNo", "Species", "SizeClass_11", "SizeClass", "Abundance", "Abundance_m2", "Biomass_kgha",  "Coloured", "Diet", "DietH", "Date", "Location", "Transect", "Observer", "Comments")) 
...
### 2024
fish.2024 <- dplyr::select(fish.2024, c("SurveyNo", "Species", "SizeClass_11", "SizeClass", "Abundance", "Abundance_m2", "Biomass_kgha",  "Coloured", "Diet", "DietH", "Date", "Location", "Transect", "Observer", "Comments"))
## Update survey numbers to be continuous
...
### 2025
fish.2025 <- dplyr::select(fish.2025, c("SurveyNo", "Species", "SizeClass_11", "SizeClass", "Abundance", "Abundance_m2", "Biomass_kgha",  "Coloured", "Diet", "DietH", "Date", "Location", "Transect", "Observer", "Comments"))
## Update survey numbers to be continuous
...
fish <- dplyr::select(fish, "SurveyNo", "Site", "Treatment", "Patch.type", "Transect", "Year.created", "Date.created", "GPS",
                            ...
                            "Species", "Diet", "DietH", "SizeClass", "Abundance", "Abundance_m2", "Biomass_kgha", "Coloured", "SizeClass_11")
...
fish$SizeClass <- as.numeric(fish$SizeClass)

#Format as wide, because .xlsx has a max of 1M rows
fish <- fish %>%
  pivot_wider(
    names_from = SizeClass,
    values_from = Abundance,
    values_fill = 0,          # Fill missing with 0
    values_fn = sum           # Aggregate duplicates by summing (Sizes above 55 occur multiple times, but are separate rows)
  )
...
rm(fish, fish.2017_2022, fish.2018, fish.2019, fish.2020, fish.2021, fishRAW.2018, fishRAW.2019, fishRAW.2020, fishRAW.2021, meta.all,
   ...
   SurveyNo2025,
   surveys.tot, obs_block, raw_obs,  input_file, wb, fish_sizeclass, co, data_summary, biomass_sum, col_order, cols, i, idx_12)
...
# Identify ID columns
id_cols <- c("SurveyNo", "Site", "Treatment", "Patch.type", "Transect", "Year.created", "Date.created", "GPS",
            "Depth.MLLW", "Reef.dist_m", "AR.dist_m", "Area_m2", "Observer", "Comments", "Year", "Date",
            "Species", "Diet", "DietH", "Abundance_m2", "Biomass_kgha", "Coloured", "SizeClass_11")
# Everything else are the size columns to pivot
value_cols <- setdiff(names(fish), id_cols) 

# Pivot to long
fish <- fish %>%
  pivot_longer(
    cols = all_of(value_cols),   # only pivot size columns
    names_to = "SizeClass",
    values_to = "Abundance"
  ) %>%
  mutate(SizeClass = as.numeric(SizeClass))  # convert Size to numeric

rm(id_cols, value_cols)

## Replace the 65-proxy by the actual size of the largest class, which varies between species / survey. If there is no fish in the largest class, keep 65 in the frame.
fish <- fish %>%
  mutate(SizeClass = ifelse(SizeClass == 65 & Abundance > 0, SizeClass_11, SizeClass)) %>%
  select(-SizeClass_11)

## Remove all duplicate rows that were caused by carrying over Biomass_kgha and Abundance_m2 when converting long > wide > long
fish <- fish %>%
  mutate(
    Abundance_m2 = if_else(Abundance == 0, 0, Abundance_m2),
    Biomass_kgha = if_else(Abundance == 0, 0, Biomass_kgha))
fish <- fish %>%
  distinct()
fish <- fish %>%
  group_by(Species, SurveyNo, SizeClass) %>%
  filter(!(n() > 1 & Abundance == 0)) %>%
  ungroup()

fish <- fish %>%
  group_by(Species, SurveyNo) %>%
  filter(
    !(SizeClass == 65 & Abundance == 0 & any(SizeClass >= 60 & Abundance > 0))
  ) %>%
  ungroup()

# Rename SizeClass to Size, considering it is not a class system now
fish <- fish %>%
    rename(Size = SizeClass)
...
exceptions <- fish %>%
  group_by(Species, SurveyNo) %>%
  summarise(n_sizes = n(), .groups = "drop") %>%
  filter(n_sizes != 11)
