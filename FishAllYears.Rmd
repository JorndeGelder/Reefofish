...
survey_meta <- Richness %>%
  select(SurveyNo, Transect, Depth.MLLW, Reefdistance, ARdistance, Reef.dist_m, AR.dist_m, YearGroup, TransectDate, Patch.type, Treatment, Observer) %>%
  distinct()

## Count distinct herbivore species per herbivore guild per survey, and fill in 0 if there are none.
Richness <- Richness %>%
  group_by(SurveyNo, DietH) %>%
  summarise(SpeciesRichness = n_distinct(Species[Abundance > 0]),
            .groups = "drop") %>%
...
Richness_combined <- Richness %>%
  ...
  summarise (SpeciesRichness = sum(SpeciesRichness))

# Correlation between reef depth and species richness
plot(Richness_combined$Depth.MLLW, Richness_combined$SpeciesRichness, xlab = "Depth", ylab = "Species richness", main = "Herbivore species richness vs Depth (covariate)")
abline(lm(SpeciesRichness ~ Depth.MLLW, data = Richness_combined))
sum_lm <- summary (lm(SpeciesRichness ~ Depth.MLLW, data = Richness_combined))
...
plot(Richness_noREF$Reef.dist_m, Richness_noREF$SpeciesRichness, xlab = "Reef distance", ylab = "Species richness", main = "Herbivore species richness vs distance from natural reef (covariate)")
abline(lm(SpeciesRichness ~ Reef.dist_m, data = Richness_noREF))
sum_lm <- summary (lm(SpeciesRichness ~ Reef.dist_m, data = Richness_noREF))
coef_lm <- sum_lm$coefficients
text(x = 23,
     y = 8,
     labels = paste0("R^2 = ",
...
plot(Richness_ARonly$AR.dist_m, Richness_ARonly$SpeciesRichness, xlab = "AR distance", ylab = "Species richness", main = "Herbivore species richness vs distance from another artifical reef (covariate)")
abline(lm(SpeciesRichness ~ AR.dist_m, data = Richness_ARonly))
sum_lm <- summary (lm(SpeciesRichness ~ AR.dist_m, data = Richness_ARonly))
coef_lm <- sum_lm$coefficients
text(x = 23,
     y = 8,
     labels = paste0("R^2 = ",
...
## Statistical models
```{r richness}
# # Version 1: Gaussian. Leads to non-normal distribution of residuals, and doesn't fit count data well.
# RmodelG<- lmer(SpeciesRichness ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Richness)
#
# # Version 2: Negative binomial. Considering underdispersion (UD) gives 0,78 it is a feasible option.
# RmodelNB <- glmmTMB(SpeciesRichness ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=nbinom2)
#
# # Version 3: Conway-Maxwell Poisson, but with dispersion formula
# RmodelCMP1 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois(), dispformula = ~ Patch.type)
#
# # Version 4: v5, but with both distances and no dispersion formula
# RmodelCMP2 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + Reefdistance:Treatment + ARdistance:Treatment + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois())

# # Version 5: Negative binomial with distances including NA values (to test significance of distance effects)
# RmodelCMP3 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + Reef.dist_m +  AR.dist_m + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois())
#
# # Version 6: v6, but with distances as non-interaction (thus 0's where irrelevant)
# RmodelCMP4 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + Reefdistance + ARdistance + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois())

# # Version 7: Conway-Maxwell Poisson, but without distances modeled.
RmodelCMP5 <- glmmTMB(SpeciesRichness ~ Depth.MLLW + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Richness, family=compois())

# # Look at model
# Anova(modelCMP4excl)
# summary(modelCMP4excl)$coefficients$cond #<- Coefficients of reef_distance and AR_distance are very small compared to others, omit
# AIC(modelCMP4, modelCMP4red) #<- Reduced model is same quality, preferred
...
cld_year <- cld(emm_year, Letters = letters, adjust = "tukey") # Letters show significance
...
ggplot(emm_df, aes(x = YearGroup, y = response, fill = DietH)) +
  ...
  scale_fill_brewer(palette = "Set2", name = "Herbivore guild") +  # legend restored
  labs(
    x = "Yeargroup",
    y = "Species richness",
...
## Pre-model plots
```{r}
# Correlation between reef depth and biomass
plot(Biomass$Depth.MLLW, Biomass$GuildBiomass, xlab = "Depth", ylab = "Biomass", main = "Biomass vs Depth (covariate)")
abline(lm(GuildBiomass ~ Depth.MLLW, data = Biomass))
sum_lm <- summary (lm(GuildBiomass ~ Depth.MLLW, data = Biomass))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 10,
     labels = paste0("R^2 = ",
...
Biomass_noREF <- Biomass[Biomass$Treatment != "Reference",] 
plot(Biomass_noREF$Reef.dist_m, Biomass_noREF$GuildBiomass, xlab = "Reef distance", ylab = "Biomass (kg/ha)", main = "Biomass vs distance from natural reef (covariate)")
abline(lm(GuildBiomass ~ Reef.dist_m, data = Biomass_noREF))
sum_lm <- summary (lm(GuildBiomass ~ Reef.dist_m, data = Biomass_noREF))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 10,
     labels = paste0("R^2 = ",
...
Biomass_ARonly <- Biomass[Biomass$Treatment == "AR",] 
plot(Biomass_ARonly$AR.dist_m, Biomass_ARonly$GuildBiomass, xlab = "AR distance", ylab = "Biomass (kg/ha)", main = "Biomass vs distance from another artifical reef (covariate)")
abline(lm(GuildBiomass ~ AR.dist_m, data = Biomass_ARonly))
sum_lm <- summary (lm(GuildBiomass ~ AR.dist_m, data = Biomass_ARonly))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 10,
     labels = paste0("R^2 = ",
...
## Statistical models
```{r}
# Version 1: Gaussian. Leads to non-normal distribution of residuals.
BmodelG<- lmer(GuildBiomass ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Biomass) 

# Version 2: log-transformed, Gaussian
BmodelOutl<- lmer(log10(1+GuildBiomass) ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Biomass) 
...
# Version 4: log-link, only taking Reefdistance into account, because it's significant in version 3.
BmodelLoglink2 <- glmmTMB(GuildBiomass ~ Depth.MLLW + Reefdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Biomass, family = tweedie(link="log"))

# Version 5: log-link, taking both distances into account
BmodelLoglink3 <- glmmTMB(GuildBiomass ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), dispformula = ~ DietH, data = Biomass, family = tweedie(link="log"))

# Version 6: v5, but with distances without Treatment & no disp
BmodelLoglink4 <- glmmTMB(GuildBiomass ~ Depth.MLLW + Reefdistance +  ARdistance + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Biomass, family = tweedie(link="log"))

# Look at model
Anova(BmodelLoglink1)
 
## Normality
resmodelG <- resid(BmodelG)
qqnorm(resmodelG)
qqline(resmodelG)

# Homoscedasticity
plot(fitted(BmodelG), resmodelG,
      xlab = "Fitted values", ylab = "Residuals")
 abline(h = 0, col = "red")
 
## Normality
resmodelOutl <- resid(BmodelOutl)
qqnorm(resmodelOutl)
qqline(resmodelOutl)

# Homoscedasticity
plot(fitted(BmodelOutl), resmodelOutl,
      xlab = "Fitted values", ylab = "Residuals")
 abline(h = 0, col = "red") 
 
# Normality & homoscedasticity for DHARMa
Bsim_resLoglink2 <- simulateResiduals(fittedModel = BmodelLoglink2, re.form = NULL)
Bsim_resLoglink3 <- simulateResiduals(fittedModel = BmodelLoglink3, re.form = NULL)
Bsim_resLoglink4 <- simulateResiduals(fittedModel = BmodelLoglink4, re.form = NULL)

# plot residuals
plot(Bsim_resLoglink2)
plot(Bsim_resLoglink3)
plot(Bsim_resLoglink4)

testCategorical(Bsim_resLoglink3, catPred = Biomass$Depth.MLLW)
testCategorical(Bsim_resLoglink3, catPred = Biomass$YearGroup)
testCategorical(Bsim_resLoglink3, catPred = Biomass$Patch.type)
testCategorical(Bsim_resLoglink3, catPred = Biomass$DietH)
testResiduals(Bsim_resLoglink3)
```
## Plots
```{r}
...
ggplot(emm_df, aes(x = YearGroup, y = response, fill = DietH)) +
  ...
  labs(
    x = "Yeargroup",
    y = "Biomass (kg/ha)",
    title = "Predicted biomass per survey"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    axis.text = element_text(color = "black"),
...
emm_df <- as.data.frame(emmeans(BmodelLoglink4, ~ YearGroup * DietH * Patch.type, type = "response"))
cld_total <- cld(emmeans(BmodelLoglink4, ~ YearGroup | Patch.type, type="response"), Letters=letters)
...
emm_totals <- emm_totals %>%
  left_join(cld_total %>% select(YearGroup, Patch.type, .group), by=c("YearGroup","Patch.type"))
...
ggplot(emm_df, aes(x = YearGroup, y = response, fill = DietH)) +
  ...
  geom_errorbar(
    data = emm_totals,
    aes(x = YearGroup, ymin = LCL, ymax = UCL, group = Patch.type),
    inherit.aes = FALSE,
    width = 0.25,
    size = 0.8
  ) +
  geom_text(
    data = emm_totals,
    aes(x = YearGroup, y = UCL + 0.10*max(UCL), label = .group),
    inherit.aes = FALSE
  ) +
  facet_wrap(~ Patch.type, ncol = 3) +
...
# Plotted  per patch type and per herbivore guild
# Guild-level predictions
emm_df <- as.data.frame(emmeans(BmodelLoglink4, ~ YearGroup * DietH * Patch.type, type = "response"))
emm_letters <- emmeans(BmodelLoglink4, ~ YearGroup | DietH + Patch.type)
cld_letters <- cld(emm_letters, Letters = letters, adjust = "tukey") %>%
  select(DietH, Patch.type, YearGroup, .group)

# Compute totals per stack and approximate SE
emm_totals2 <- emm_df %>%
  group_by(DietH, Patch.type, YearGroup) %>%
  summarise(
    total = sum(response),                # sum of predicted means per stack
    SE_total = sqrt(sum(SE^2)),           # approximate SE
    .groups = "drop"
  ) %>%
  mutate(
    LCL = total - 1.96*SE_total,
...
emm_totals2 <- emm_totals2 %>%
  left_join(cld_letters, by = c("DietH", "Patch.type", "YearGroup"))

ggplot(emm_totals2, aes(x = YearGroup, y = total)) +
  ...
  geom_text(
    aes(y = UCL + 0.5, label = .group),  # small offset above error bar
    vjust = 0 
  ) +
  facet_grid(DietH ~ Patch.type) +
  labs(
    x = "YearGroup",
    y = "Biomass (kg/ha)",
    title = "Predicted biomass per survey"
...
# Comparing guilds across patch types
...
ggplot(emm_guild_patch, aes(x = Patch.type, y = total, fill = Patch.type)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_errorbar(aes(ymin = LCL, ymax = UCL), width = 0.2, size = 0.8) +
  facet_wrap(~ DietH, ncol = 1) +   # one row per guild
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"   # optional: remove legend since Patch.type is on x-axis
  )
...
# Three bar plots 
emm_guild_patch_year <- emm_df %>%
  group_by(YearGroup, DietH, Patch.type) %>%
  summarise(
    total = sum(response),
    SE_total = sqrt(sum(SE^2)),  # approximate SE for sum
    .groups = "drop"
...
emm_letters <- emmeans(BmodelLoglink4, ~ Patch.type | DietH + YearGroup, type="response")
cld_letters <- cld(emm_letters, Letters = letters, adjust = "tukey") %>%
  select(YearGroup, DietH, Patch.type, .group)

emm_guild_patch_year <- emm_guild_patch_year %>%
  left_join(cld_letters, by = c("YearGroup", "DietH", "Patch.type"))

ggplot(emm_guild_patch_year, aes(x = DietH, y = total, fill = Patch.type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  geom_errorbar(
    aes(ymin = LCL, ymax = UCL),
    position = position_dodge(width = 0.8),
...
plot(Meansize$Depth.MLLW, Meansize$GuildMeansize, xlab = "Depth", ylab = "Mean size (cm)", main = "Mean size vs Depth (covariate)")
abline(lm(GuildMeansize ~ Depth.MLLW, data = Meansize))
sum_lm <- summary (lm(GuildMeansize ~ Depth.MLLW, data = Meansize))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 10,
     labels = paste0("R^2 = ",
...
Meansize_noREF <- Meansize[Meansize$Treatment != "Reference",] 
plot(Meansize_noREF$Reef.dist_m, Meansize_noREF$GuildMeansize, xlab = "Reef distance", ylab = "Mean size (cm)", main = "Mean herbivore fish size vs distance from natural reef (covariate)")
abline(lm(GuildMeansize ~ Reef.dist_m, data = Meansize_noREF))
sum_lm <- summary (lm(GuildMeansize ~ Reef.dist_m, data = Meansize_noREF))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 10,
     labels = paste0("R^2 = ",
...
Meansize_ARonly <- Meansize[Meansize$Treatment == "AR",] 
plot(Meansize_ARonly$AR.dist_m, Meansize_ARonly$GuildMeansize, xlab = "AR distance", ylab = "Mean size (cm)", main = "Mean herbivore fish size vs distance from another artifical reef (covariate)")
abline(lm(GuildMeansize ~ AR.dist_m, data = Meansize_ARonly))
sum_lm <- summary (lm(GuildMeansize ~ AR.dist_m, data = Meansize_ARonly))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 10,
     labels = paste0("R^2 = ",
...
```{r}
# Version 1: Gaussian. Leads to non-normal distribution of residuals.
MSmodelG<- lmer(GuildMeansize ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Meansize) 

# Version 2: Gaussian with log-transformation.
MSmodelOutl<- lmer(log10(1+GuildMeansize) ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Meansize) 
...
# Version 4: Log-link, excluding ARdist as it is insignificant in v3.
MSmodelLoglink2 <- glmmTMB(GuildMeansize ~ Depth.MLLW + Reefdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Meansize, family = tweedie(link="log"))

# Version 5: Log-link, both distances included
MSmodelLoglink3 <- glmmTMB(GuildMeansize ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1| TransectDate:Observer), data = Meansize, family = tweedie(link="log"))

# Version 6: Log-link, both distances included as non-interaction
MSmodelLoglink4 <- glmmTMB(GuildMeansize ~ Depth.MLLW + Reefdistance +  ARdistance + YearGroup * Patch.type + DietH + (1 | Transect) + (1| TransectDate:Observer), data = Meansize, family = tweedie(link="log"))

# Look at model
Anova(MSmodelLoglink1)

## Normality
MSresmodelG <- resid(MSmodelG)
qqnorm(MSresmodelG)
qqline(MSresmodelG)

# Homoscedasticity
plot(fitted(MSmodelG), MSresmodelG,
      xlab = "Fitted values", ylab = "Residuals")
 abline(h = 0, col = "red")
 
# Normality
MSresOutl <- resid(MSmodelOutl)
qqnorm(MSresOutl)
qqline(MSresOutl)

# Homoscedasticity
plot(fitted(MSmodelOutl), MSresOutl)
abline(h = 0, lwd = 2, col = "red")
 
# Normality & homoscedasticity for DHARMa
MSsim_resLoglink2 <- simulateResiduals(fittedModel = MSmodelLoglink2, re.form = NULL)
MSsim_resLoglink3 <- simulateResiduals(fittedModel = MSmodelLoglink3, re.form = NULL)
MSsim_resLoglink4 <- simulateResiduals(fittedModel = MSmodelLoglink4, re.form = NULL)

# plot residuals
plot(MSsim_resLoglink2)
plot(MSsim_resLoglink3)
plot(MSsim_resLoglink4)

testResiduals(MSsim_resLoglink2)
testResiduals(MSsim_resLoglink3)
...
Abundance <- Abundance %>%
  group_by(SurveyNo, DietH) %>%
  summarise(SpeciesAbundance = sum(Abundance),
            .groups = "drop") %>%
  right_join(all_combos, by = c("SurveyNo", "DietH")) 
...
AmodelNB1 <- glmmTMB(SpeciesAbundance ~ Depth.MLLW + Reef.dist_m + AR.dist_m + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Abundance, family = nbinom2)

# Version 2: Negative binomial. 
AmodelNB2 <- glmmTMB(SpeciesAbundance ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data=Abundance, family=nbinom2) 

# Version 3: v2 with dispersion of DietH accounted for
AmodelNB3 <- glmmTMB(SpeciesAbundance ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Abundance, family = nbinom2, dispformula = ~ DietH)

# Version 4: v3 with zero-inflation accounted for
AmodelNB4 <- glmmTMB(SpeciesAbundance ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Abundance, ziformula = ~ 1, family = nbinom2, dispformula = ~ DietH)

# Version 5: v3 with distances as non-interaction
AmodelNB5 <- glmmTMB(SpeciesAbundance ~ Depth.MLLW + Reefdistance +  ARdistance + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Abundance, family = nbinom2, dispformula = ~ DietH)

# Version 6: Poisson
AmodelPoisson <- glmmTMB(SpeciesAbundance ~ Depth.MLLW + Reefdistance:Treatment +  ARdistance:Treatment + YearGroup * Patch.type + DietH + (1 | Transect) + (1 | TransectDate:Observer), data = Abundance, family = poisson, dispformula = ~ DietH)

# Look at model
Anova(AmodelNB1)

# Normality, heteroscedasticity with DHARMa
Asim_resNB2 <- simulateResiduals(fittedModel = AmodelNB2, re.form = NULL)
Asim_resNB3 <- simulateResiduals(fittedModel = AmodelNB3, re.form = NULL)
Asim_resNB4 <- simulateResiduals(fittedModel = AmodelNB4, re.form = NULL)
Asim_resNB5 <- simulateResiduals(fittedModel = AmodelNB5, re.form = NULL)
Asim_resPoisson <- simulateResiduals(fittedModel = AmodelPoisson, re.form = NULL)

plot(Asim_resNB2)
plot(Asim_resNB3)
plot(Asim_resNB4)
plot(Asim_resNB5)
plot(Asim_resPoisson)

testCategorical(Asim_resNB5, catPred = Abundance$Depth.MLLW)
testCategorical(Asim_resNB5, catPred = Abundance$YearGroup)
testCategorical(Asim_resNB5, catPred = Abundance$Patch.type)
testCategorical(Asim_resNB5, catPred = Abundance$DietH)
```
## Plots
```{r}
# Testing diagnostics for model 1
## Linearity of only continuous variable
Abundance_combined <- Abundance %>% 
  group_by(SurveyNo, Depth.MLLW) %>% 
  summarise (SpeciesAbundance = sum(SpeciesAbundance))

plot(Abundance_combined$Depth.MLLW, Abundance_combined$SpeciesAbundance, xlab = "Depth", ylab = "Species Abundance", main = "Species Abundance vs Depth (covariate)")
abline(lm(SpeciesAbundance ~ Depth.MLLW, data = Abundance_combined))
sum_lm <- summary (lm(SpeciesAbundance ~ Depth.MLLW, data = Abundance_combined))
coef_lm <- sum_lm$coefficients
text(x = 7,
     y = 150,
     labels = paste0("R^2 = ",
...
survey_meta <- BiomassNMDS %>%
  select(SurveyNo, Transect, Depth.MLLW, Reef.dist_m, AR.dist_m, YearGroup, TransectDate, Patch.type, Treatment, Observer) %>%
  distinct()
...
# Identify surveys with no herbivorous fish & Remove
...
BiomassNMDS_data <- BiomassNMDS[2:46]
rm(survey_meta)
...
data.scores$Patch.type = BiomassNMDS$Patch.type
coords <- as.data.frame(nmds$points)

# Get two samples with highest NMDS1 values:
outliers1 <- coords[order(coords$MDS1, decreasing = TRUE)[1:2], ]
outliers2 <- coords[order(coords$MDS2, decreasing = TRUE)[1:1], ]
...
ggplot(data.scores,aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_text(
  data = subset(data.scores, SampleID %in% c("240","304","305")),
  aes(label = SampleID),
...
# Ordination plots with outlier excluded
```{r}
# ---- DATA PREP, similar to Biomass ----
# Remove rows with non-herbivorous fish, and non-instantaneous observations
BiomassNMDS <- fish[fish$DietH != "Other" & fish$Coloured == 12,]
...
## Summarize all size classes into one row
BiomassNMDS <- BiomassNMDS %>%
  group_by(SurveyNo, Species) %>%
  summarise(Biomass = sum(Biomass_kgha),
            .groups = "drop")
...
# Identify surveys with no herbivorous fish & Remove >> Otherwise vegdist doesn't work
empty_rows <- rowSums(BiomassNMDS[,-1]) == 0
empty_cols <- colSums(BiomassNMDS[,-1]) == 0
BiomassNMDS <- BiomassNMDS[ !empty_rows, ]
BiomassNMDS <- BiomassNMDS[ , c(TRUE, !empty_cols) ]
...
# BiomassNMDS_data1 <- decostand(BiomassNMDS_data, "log")      # log transform <- isn't improved
# BiomassNMDS_data2 <- sqrt(BiomassNMDS_data)                  # square root <- isn't improved
# BiomassNMDS_data3 <- decostand(BiomassNMDS_data, "hellinger") # also no improvement
...
data_ell <- data.scores
data_ell$Patch.type[data_ell$SampleID %in% c("240","304","305")] <- NA  # mask them, don't drop
...
ggplot(subset(data.scores, !(SampleID %in% c("240","304","305"))), 
        aes(x = NMDS1, y = NMDS2)) +
        geom_point(size = 3, aes(colour = Patch.type)) +
