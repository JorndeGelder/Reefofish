---
title: Msc Thesis: Low mean size and stable biomass of key herbivore guilds after bleaching across artificial reef designs in Kenya
...
library(readxl) # get data from Excel
library(openxlsx) # write Excel
library(tidyxl) # Read in coloured cells Excel
library(janitor) # Excel functions
library(data.table) # converting list to data
library(arrow) # Write & read Parquet

library(tidyverse) # summarizing / selecting / filtering
library(glmmTMB) #negative binomial
library(car) # Anova for model validation
library(DHARMa) #for checking NB & CMP GLMM
library(emmeans) # estimated marginal means for modelling
...
library(ggpattern) # patterns for colorblindness
library(ggrepel) # separating texts if overlapping
```
...
Aplot3 <- ggplot(emm_patch_df, aes(x = Patch.type, y = response*100, colour = YearGroup, shape = YearGroup, group = YearGroup)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = asymp.LCL*100L, ymax = asymp.UCL*100),
                position = position_dodge(width = 0.5),
                width = 0.2) +
  geom_text(aes(label = .group, y = asymp.UCL*100),
            position = position_dodge(width = 0.5),
            ...
            size = 4.5,
            show.legend = FALSE) +
  labs(x = "Reef design", y = expression(Abundance ~ (m^-2) %*% 100),
...
# Datapoint 147 heavily distorts all other points due to anomaly
...
## PNG
png(file.path("C:/Users/Jorn de Gelder/OneDrive - Wageningen University & Research/Thesis/Plots/plots_png", "outlierplot.png"),
    width = 7, height = 5, units = "in", res = 600)
print(outlierDesign)
dev.off()

## Tiff
tiff(file.path("C:/Users/Jorn de Gelder/OneDrive - Wageningen University & Research/Thesis/Plots/tiff", "outlierplot.tiff"),
     width = 8, height = 6, units = "in", res = 600, compression = "lzw")
print(outlierDesign)
dev.off()
